...
fn try_verify_farm_token(
    ...
    let payload_bytes = engine
        .decode(payload_b64)
        .map_err(|_| (StatusCode::UNAUTHORIZED, "Invalid farm token encoding".to_string()))?;
    let sig_bytes = engine
        .decode(sig_b64)
        .map_err(|_| (StatusCode::UNAUTHORIZED, "Invalid farm token signature encoding".to_string()))?;

    // Verify Ed25519 signature via the identity crate helper.
    voxply_identity::verify_signature(farm_pubkey_hex, &payload_bytes, &sig_bytes)
        .map_err(|_| (StatusCode::UNAUTHORIZED, "Invalid farm token signature".to_string()))?;

    // Deserialise after signature check.
    let payload: FarmTokenPayload = serde_json::from_slice(&payload_bytes)
        .map_err(|_| (StatusCode::UNAUTHORIZED, "Malformed farm token payload".to_string()))?;
    ...
    let now = std::time::SystemTime::now()
        ...
        .as_secs() as i64;
    if now >= payload.exp {
        return Err((StatusCode::UNAUTHORIZED, "Farm token has expired".to_string()));
    }
...
impl FromRequestParts<Arc<AppState>> for AuthUser {
    ...
    async fn from_request_parts(
        ...
        // Ensure the users row exists (upsert last_seen_at).
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs() as i64;
        ...
        if token.contains('.') {
            ...
            if has_roles == 0 {
                // Give owner role if nobody has it yet; otherwise everyone.
